station-event-meteor-swarm-announcement = Meteors are on a collision course with the station. Brace for impact.
station-event-meteor-swarm-complete-announcement = The meteor swarm has passed. Please return to your stations.
station-event-meteor-swarm-start-announcement = Обнаружен рой метеоритов прямо по курсу станции.
station-event-meteor-swarm-end-announcement = Метеорный рой миновал. Пожалуйста, вернитесь на свои рабочие места.
station-event-space-dust-start-announcement = Станция проходит через облако обломков, ожидается незначительный ущерб элементам конструкции и внешнему оборудованию.
station-event-meteor-urist-start-announcement = Станция столкнулась с неидентифицированным облаком обломков. Пожалуйста, сохраняйте спокойствие и не слушайте их.
station-event-meteor-cow-start-announcement = Станция сталкивается с неопознанным скоплением обломков. Предупреждение: не кормите их пшеницей. Уходите подальше от внешних секций.
station-event-meteor-clown-start-announcement = Станция сталкивается с неопознанным роем обломков. ХОНК! Немедленно эвакуируйте хранилище закиси азота.
station-event-meteor-potato-start-announcement = Станция сталкивается с неопознанным роем обломков. Обнаружено высокое содержание крахмала.
station-event-fun-swarm-start-announcement = Станция сталкивается с неопознанным роем обломков. Пожалуйста, паникуйте в меру.
